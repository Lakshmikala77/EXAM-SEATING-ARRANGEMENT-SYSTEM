...
<a href="mainblock.php" class="btn  btn-sm"><span class="glyphicon glyphicon-chevron-left"></span>back</a>
  ...
  <?php
         $branchq="SELECT distinct department from trisub2 where classroom='$room'";//distinct department selection
        $retvalbranch=mysqli_query($conn,$branchq);
        $yearq="SELECT distinct year from trisub2 where classroom='$room'";//distinct year selection
...
<center><h2 class="table">Room_NO-<?php echo($room)?></h2></center>
  <div>
   ...
   <td class="main-table-data"><lable>BRANCH :<label></td>
      <td class="main-table-data">
        ...
        <?php
        $subq1="SELECT distinct subject,examdate,starttime,endtime from trisub2 where classroom='$room'";
 ...
 <div>
   <table class="Room-table">
      ...
      <?php
      $branchq1=mysqli_query($conn, "SELECT distinct department from trisub2 where classroom='$room'");
      $res = mysqli_fetch_assoc($branchq1);
      $brnh = $res['department'];
      $numq1="SELECT  enrollnumber,department from trisub2 where classroom='$room' AND department = '$brnh' ";
      $numq2="SELECT  enrollnumber,department from trisub2 where classroom='$room' And department != '$brnh'";
      ...
      <tr class="main-row">
        ...
        <tr class="main-row"><?php $count=1;$count1=1; ?>
          ...
          <table class="Room-table">
            ...
            <tr class="main-row">
              <td height="80" class="main-data "><center><?php $no1=$rownum2['enrollnumber']; echo $no1;  ?></center><hr>
